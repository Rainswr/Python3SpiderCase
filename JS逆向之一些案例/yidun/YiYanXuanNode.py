...
class YiDunecard:
    ...
    def get_js_cb(self):
        url = f"http://127.0.0.1:8082/get_cb"
        resp = requests.get(url, headers=self.user_agent, timeout=5)
        return resp.text

    def get_js_fp(self):
        url = f"http://127.0.0.1:8082/get_fp"
        resp = requests.get(url, headers=self.user_agent, timeout=5)
        return resp.text

    def get_js_data(self, offset, token, xyt):
        url = f"http://127.0.0.1:8082/get_data?xyt={xyt}&token={token}&offset={offset}"
        resp = requests.get(url, headers=self.user_agent, timeout=5)
        return resp.text

    def get_js_param_bd(self, resp_tid, resp_dt):
        url = f"http://127.0.0.1:8082/get_bd?e={resp_tid}&g={resp_dt}"
        resp = requests.get(url, headers=self.user_agent, timeout=5)
        return resp.text

    def get_js_param_dd(self):
        url = f"http://127.0.0.1:8082/get_dd"
        resp = requests.get(url, headers=self.user_agent, timeout=5)
        return resp.text

    def get_js_actoken(self, resp_dt):
        url = f"http://127.0.0.1:8082/get_actoken?d={resp_dt}"
        resp = requests.get(url, headers=self.user_agent, timeout=5)
        return resp.text
